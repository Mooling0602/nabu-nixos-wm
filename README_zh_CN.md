...
| 版本 | `nixosConfigurations` 键 | 主机名 | 根文件系统 |
| --- | --- | --- | --- |
| `ext4` | `ext4-nabu`（别名 `nabu`，默认） | `nabu` | ext4 |
| `impermanent` | `impermanent-nabu` | `impermanent-nabu` | tmpfs 根 + Btrfs 持久化（实验性） |

每个系统的主机名与其 `nixosConfigurations` 键一致，因此直接执行
`nixos-rebuild switch --flake .` 会选中当前实际安装的配置。
存储与持久化设置见[存储方案](docs/storage.md)；无状态版本仍需实机验证。
...
当前有两个 NixOS 配置：`ext4-nabu`（别名为 `nabu`，默认）与 `impermanent-nabu`，
均包含 niri + Noctalia。每个版本提供 `<版本>-nabu-esp`、`<版本>-nabu-rootfs` 的
`x86_64-linux` 与 `aarch64-linux` 输出；`nabu-esp`、`nabu-rootfs`、`nabu-kernel`
保留为 ext4 入口，默认输出为 ESP。
